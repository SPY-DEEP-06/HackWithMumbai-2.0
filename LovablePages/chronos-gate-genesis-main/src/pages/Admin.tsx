...
const Admin = () => {
  ...
  const handleQRVerify = (scannedData: string) => {
    try {
      ...
      const team = teams.find((t) => t.uniqueHexId === data.uniqueId);
      if (team) {
        setScanResult(`✓ Valid: ${team.teamName} (${team.uniqueHexId})`);
      } else {
  ...
  return (
    <div className="min-h-screen relative overflow-hidden">
      ...
      <div className="relative z-10 min-h-screen">
        ...
        <div className="max-w-7xl mx-auto px-4 py-8 admin-content">
          {/* Metrics */}
              ...
              <div className="flex gap-2">
                {['all', 'paid', 'pending'].map((f) => (
                  <button
                    key={f}
                    onClick={() => setFilter(f)}
                    className={`px-4 py-2 rounded-lg font-courier text-sm capitalize ${
                      filter === f
                        ? 'bg-hackathon-orange text-black'
                        : 'bg-black/50 text-gray-400 border border-hackathon-orange/30'
                    }`}
                  >
                    {f}
                  </button>
                  ...
                  {loading ? (
                    ...
                    filteredTeams.map((team) => (
                      <tr key={team.id} className="border-b border-hackathon-orange/10 hover:bg-hackathon-orange/5">
                        ...
                        <td className="p-4">
                          <span
                            className={`px-2 py-1 rounded text-xs font-courier ${
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80">
            <div className="tva-terminal max-w-md w-full p-6">
              <div className="flex justify-between items-center mb-6 border-b border-hackathon-orange/30 pb-4">
                ...
                <button onClick={() => { setShowScanner(false); setScanResult(null); }} className="text-gray-400 hover:text-white">
                  <X className="w-6 h-6" />
                </button>
              </div>

              <div className="space-y-4">
                <p className="text-gray-400 font-courier text-sm text-center">
                  Enter or paste QR data to verify
                </p>
                <textarea
                  className="futuristic-input w-full h-32"
                  placeholder="Paste QR code data here..."
                  onChange={(e) => handleQRVerify(e.target.value)}
                />
                {scanResult && (
                  <div className={`p-4 rounded-lg font-courier ${
                    scanResult.includes('Valid') 
                      ? 'bg-hackathon-loki/20 text-hackathon-loki' 
                      : 'bg-hackathon-scarlet/20 text-hackathon-scarlet'
                  }`}>
                    {scanResult}
                  </div>
                )}
